...
import { HiAcademicCap, HiBookOpen, HiClipboardList, HiCollection, HiUserGroup } from "react-icons/hi";
...
export function ExamTable({ data, role, allLessons }: ExamTableProps) {
  const baseColumns: ColumnDef<ExamList>[] = [
    {
      accessorKey: "title",
      header: ({ column }) => {
        return (
          ...
          >
            <HiClipboardList className="mr-2 h-4 w-4 text-blue-600" />
            Judul
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-3">
          <div className="p-2 bg-blue-100 dark:bg-blue-900/30 rounded-lg">
            <HiClipboardList className="h-4 w-4 text-blue-600 dark:text-blue-400" />
          </div>
        ...
        <div className="hidden md:flex justify-center">
          <div className="hidden md:flex">
            ...
            >
              ...
              {row.original.lesson.class.grade.level === 1
                ? "X"
                : row.original.lesson.class.grade.level === 2
                ? "XI"
                : "XII"}{" "}
      ...
      cell: ({ row }) => (
        <div className="hidden md:flex justify-center">
          <div className="hidden xl:flex items-center gap-2">
            ...
            >
              <Clock className="mr-1 h-3 w-3" />
              {new Date(row.original.endTime).toLocaleString("id-ID", {
                year: "numeric",
                month: "short",
                day: "numeric",
                hour: "2-digit",
  ...
  const adminColumns: ColumnDef<ExamList>[] = [
    {
      id: "actions",
      header: () => (
