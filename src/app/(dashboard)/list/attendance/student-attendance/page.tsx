...
const StudentAttendancePage = async () => {
  ...
  // Statistik absensi
  const hadir = student.attendances.filter(
    (a) => a.status === "PRESENT"
  ).length;
  const izin = student.attendances.filter(
    (a) => a.status === "PERMITTED"
  ).length;
  const sakit = student.attendances.filter((a) => a.status === "SICK").length;
  const tanpaKeterangan = student.attendances.filter(
    (a) => a.status === "ABSENT"
  ).length;
              ...
              {lessonIds.length > 0 ? (
                <div className="space-y-4">
                  {lessonIds.map((lessonId: string) => {
                    const { lesson, attendances } =
                      attendancesByLesson[lessonId];
